...
public class Update extends Insert {
    ...
    @Override
     public void request(String columns, String enteredValues) {
        ...
        String columnsInRequest = super.columns(super.table.getColumns(), " = ?, ");
        ...
        if (dbManager.isExists(Integer.parseInt(enteredData[0]))){
            try{
                dbManager.update(idColunm, changedColumns, super.table, super.enteredData);
                wiev.write(String.format("Table %s was updated", tableName));
            }catch (SQLException e){
                wiev.error("Can't update the table\n", e);
            }
        }
        else {
            wiev.write("Can't update the table, row with entered " + super.table.getColumns()[0] + " not exist");
        }
    }
